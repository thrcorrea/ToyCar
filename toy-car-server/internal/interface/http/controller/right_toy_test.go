...
func TestRightToyCar(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockBehaviour: func() {
				toyCarMock.EXPECT().Right().Return(toy.ErrCantMoveToyCarOffBoard).Once()
			},
			wantErr:    true,
		...
		},
		{
			...
			mockBehaviour: func() {
				toyCarMock.EXPECT().Right().Return(nil).Once()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/toy/right", nil)
			...
			if err := RightToyCar(tt.args.ctx, tt.args.toyCar)(c); (err != nil) != tt.wantErr {
				t.Errorf("RightToyCar() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
