...
func TestMoveToyCar(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockBehaviour: func() {
				toyCarMock.EXPECT().Move().Return(toy.ErrCantMoveToyCarOffBoard).Once()
			},
			wantErr:    true,
		...
		},
		{
			...
			mockBehaviour: func() {
				toyCarMock.EXPECT().Move().Return(nil).Once()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/toy/move", nil)
			...
			if err := MoveToyCar(tt.args.ctx, tt.args.toyCar)(c); (err != nil) != tt.wantErr {
				t.Errorf("MoveToyCar() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
